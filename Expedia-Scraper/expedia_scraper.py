...
def parse(source,destination,dateout,datein):
	
	for i in range(5):
		try:
			url = "https://www.expedia.com.sg/Flights-Search?flight-type=on&starDate={2}&endDate={3}&mode=search&trip=roundtrip&leg1=from:{0},to:{1},departure:{2}TANYT&leg2=from:{1},to:{0},departure:{3}TANYT&passengers=adults:1&options=maxhops:1".format(source,destination,dateout,datein)
...
if __name__=="__main__":
	...
	argparser.add_argument('dateout',help = 'DD/MM/YYYY')
	argparser.add_argument('datein',help = 'DD/MM/YYYY')
	...
	scraped_data = parse(source,destination,dateout,datein)
